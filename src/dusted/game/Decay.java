...
public class Decay implements ApplicationListener {
    //AI shields will ignore these units
    public Seq<UnitType> ignoreShield = new Seq<>();
    public ObjectFloatMap<UnlockableContent> decayMultipliers = new ObjectFloatMap<>();
    ...
    public void update() {
        ...
        if (Vars.state.isPlaying() && !Vars.state.isEditor()) {
            Groups.unit.each(u -> {
                ...
                float dmg = (decayDamage + (u.isGrounded() && !u.hovering ? u.tileOn().floor().attributes.get(DustedAttribute.decay) : 0f)) * decayMultipliers.get(u.type, 1f) * Time.delta;
